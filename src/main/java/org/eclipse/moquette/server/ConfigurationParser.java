...
class ConfigurationParser {
    ...
    void parse(File file) throws ParseException {
        if (file == null) {
            System.out.println("Moquette: parsing NULL file, so fallback on default configuration!");
            return;
        }
        if (!file.exists()) {
            System.out.println("Moquette: " + String.format("parsing not existing file %s, so fallback on default configuration!", file.getAbsolutePath()));
            return;
        }
        try {
            ...
            System.out.println("Moquette:  " + String.format("parsing not existing file %s, so fallback on default configuration!", file.getAbsolutePath() + fex));
            return;
        }
    }
    ...
    void parse(Reader reader) throws ParseException {
        if (reader == null) {
            ...
            System.out.println("Moquette: parsing NULL reader, so fallback on default configuration!");
            return;
        }
